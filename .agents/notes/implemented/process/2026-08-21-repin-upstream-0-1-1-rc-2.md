...
Lessons, index entries, and lab fixtures describe the same tree a reader gets from `npm pack @deepseek-ai/dsh@0.1.1-rc.2`. A dated scoped semantic review lives in [lesson 18](../../../../study/18-维护更新与版本迁移.md): agent-loop cancellation finalization, the new team session-event types, the llm `prepareCall` generation binding, code-mode image deferral, and the app-boot env blocklist were each checked against the lessons that cite them, with fixes landed where wording had drifted. Surfaces outside that review (vendor, apps/web, python SDK, and everything needing a real run) remain unreviewed, and the homepage baseline line keeps that boundary visible to readers.
