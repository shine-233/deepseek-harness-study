...
- 精灵数据存放在 `website/public/mascot-sprite.js`，是两个消费方的唯一来源：伴侣在运行时导入它，`JournalHome.vue` 在构建期打包同一份文件，静态首屏不缺精灵（22×21 字符贪婪合并成 SVG rect，眼睛格打标以驱动 CSS 眨眼循环）。首页组件不再持有私有副本。
...
**立刻让 JournalHome 与伴侣共享同一份精灵模块。** 当初暂缓：JournalHome 正处于并行工作流中，五十行数据重复比在别人文件里制造合并冲突便宜。后已在 `website/public/mascot-sprite.js` 完成。合并时否决了 Vue 侧运行时动态导入的方案——VitePress 在构建期预渲染页面，仅浏览器端的动态导入会让精灵在 hydration 前空缺；按相对路径打包 public 文件保住了单一来源和静态首屏。
