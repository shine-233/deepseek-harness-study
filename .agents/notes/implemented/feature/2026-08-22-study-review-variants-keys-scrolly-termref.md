...
8. **Third linking instance** (follow-up): the research-debug bridge chart note turns its four outcome names into buttons that flash the matching flow-band label; tool-visibility hero terms 已注册 / 对模型可见 / 允许执行 became focusable buttons that flash the matching metric card (`.term-flash`, shared shell style).
