...
Nine more labs ship the same tamper experiment beyond the original four flagships, each with a single-cause fault whose red points at exactly one check:
...
- `orchestration-lab.html` — per-mode faults: `replay-missed` zeroes the skipped-anchor ledger while the overdue step stays in the timeline (`CATCHUP_LATEST_ONLY`; ineffective on live-idle, where nothing was missed); `drop-agent-end` removes the last `workflow/agent-end` (`AGENT_PAIRING`; ineffective on the cancelled path, where synthesized ends are load-bearing).
- `worker-protocol-lab.html` — `drop-child-reply`: the first `ChildStarted`/`ChildStartError` reply disappears. `CHILD_RPC_PAIRED` reports the unclaimed child; ineffective on cancel-mid-flight, which carries no RPC.
- `code-run-lab.html` — `silent-stringify`: on any failure scenario the absent completion value is replaced by a forged string. `LOSSLESS_JSON_BOUNDARY` catches exactly the substitution the spec forbids; ineffective on success (nothing absent) and blocked-before-run.

All thirteen follow the flagship mechanics: the fault is a validated model input (`none` is the only default; unknown types throw at the model boundary), the oracle keeps recomputing its rules from raw state, and the fault select participates in each page's `#state=` link schema where the page has one (plan-stack and orchestration never shipped state links). The fault note names the tripped check id and why that check exists.

## Consequences

The tamper-experiment pattern now spans thirteen labs covering seven violation shapes: dropped evidence (turn-flow, compaction, worker-protocol), forged evidence (session-fork), swallowed evidence (invariant, llm-stream), inconsistent evidence (tool-visibility, plan-stack todo/goal), overstepping authority (guard-loop), rejected work leaking through (subagent-delegate, orchestration workflow), forged conclusion fields (session-log, plan-stack plan), and forged values (code-run). Remaining labs still have tautological oracles; extending them needs a per-model decision about which invariant a reader should break, not more shared machinery.

Faults mutate built model objects (`applyXFault`) rather than threading through every build helper, so each model owns one mutation site. A future model change that renames mutated fields must update the fault appliers in the same change. Single-cause discipline constrains where a fault may fire: guard-loop refuses injections that would dirty two ledgers at once, orchestration refuses them where synthesis accounting is load-bearing.

## Alternatives considered

**A generic fault framework driven by per-lab config.** Rejected for now: each lab's teachable violation differs in shape (drop a log write vs forge a result vs blank an error vs block a call vs fake a value), so a shared engine would need per-lab hooks everywhere while hiding the single mutation site this design relies on. Revisit only when another shape appears.
...
Model tests assert the exact red-check set per fault plus ineffective-case green and loud failure on unknown fault types (`session-fork.test.mjs` 11 pass, `invariant.test.mjs` 9 pass, `tool-visibility.test.mjs` 15 pass, `guard-loop.test.mjs` 19 pass, `subagent-delegate.test.mjs` 15 pass, `plan-stack.test.mjs` 4 pass, `orchestration.test.mjs` 13 pass, `worker-protocol.test.mjs` 11 pass, `code-run.test.mjs` 9 pass). Playwright acceptance drives all nine pages end to end (ten entries — orchestration runs twice): baseline green, injected red with the note naming the rule, ineffective combination green with the explanation visible, revert to green, and state-link presence where the page ships it. Full smoke passes 9/9 and `pnpm run website:build` stays five-green.
