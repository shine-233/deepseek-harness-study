...
test('生成题的答案能从模型输出独立推导，id 稳定，选项无重复', () => {
  ...
  const questions = generatedQuestionsFor('04-Agent与Turn流程')
    .filter(question => question.id.startsWith('gen-04-lane-'))
  const ids = new Set()
...
test('Session 读数题的数值来自重放输出，干扰项不吞掉正确项', () => {
  for (const question of generatedQuestionsFor('05-Session日志与恢复')) {
    if (!question.id.startsWith('gen-05-tools-')) continue
...
test('模型请求计数与缺口停位题同样能从模型独立推导', () => {
  const requests = generatedQuestionsFor('04-Agent与Turn流程')
    .find(question => question.id === 'gen-04-requests-two-tools')
  assert.notEqual(requests, undefined)
  const requestTruth = buildTurnModel({ scenario: 'two-tools' }).observations.modelRequests
  assert.equal(requests.options[requests.answer], String(requestTruth) + ' 次')

  const refusal = generatedQuestionsFor('05-Session日志与恢复')
    .find(question => question.id === 'gen-05-refusal-gap')
  assert.notEqual(refusal, undefined)
  const refusalModel = buildSessionLogModel({ scenario: 'gap' })
  assert.ok(refusalModel.observations.refusedAt !== null, 'gap 场景应当拒绝')
  assert.equal(refusal.options[refusal.answer], '序号 ' + String(refusalModel.observations.refusedAt))
})

test('allQuestionsFor 是手写题库加生成题；手写题库本体不动', () => {
  const combined = allQuestionsFor('04-Agent与Turn流程')
  assert.equal(combined.length, QUIZ_BANK['04-Agent与Turn流程'].length + 3)
  assert.equal(allQuestionsFor('05-Session日志与恢复').length, QUIZ_BANK['05-Session日志与恢复'].length + 3)
