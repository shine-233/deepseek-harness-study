...
- 所属层：仓库根部的构建、测试或开发入口
- 文件角色：功能实现
- 这个文件有什么用：它负责 `vitest.shared.ts`；固定提交中扫描到的公开或顶层声明包括 `vitestExecArgv`、`standardDecoratorPlugin`。把这块责任留在自己的层里，可以让调用关系清楚，修改时只影响需要它的部分。
- 为什么这样设计：固定提交中它与 0 个相对依赖和 5 个直接使用者相连；保持这个文件职责较窄，可以让依赖方向和替换边界清楚。
