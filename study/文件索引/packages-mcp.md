...
- 文件角色：功能实现
- 这个文件有什么用：它负责 MCP 连接、浏览器端；文件顶部注释把它定位为“Connection supervisor: owns the MCP client/transport generations for one plugin instance, keeps the harness tool registry in sync with the live generation, and — when the connection drops — restarts the configured server with bounded exponential backoff. On...”。固定提交中扫描到的公开或顶层声明包括 `ReconnectConfig`、`RECONNECT_DEFAULTS`、`ResolvedReconnectPolicy`、`resolveReconnectPolicy`。把这块责任留在自己的层里，可以让调用关系清楚，修改时只影响需要它的部分。
- 为什么这样设计：固定提交的文件顶部注释把它定位为“Connection supervisor: owns the MCP client/transport generations for one plugin instance, keeps the harness tool registry in sync with the live generation, and — when the connection drops — restarts the configured server with bounded exponential backoff. On...”；把这项职责单独放置，可以让边界、输入和失败处理在一个地方被阅读和测试。
...
- 代码证据：固定提交归档实际读取结果：约 351 行；扫描到的声明包括 `ReconnectConfig`、`RECONNECT_DEFAULTS`、`ResolvedReconnectPolicy`、`resolveReconnectPolicy`、`ConnectionOutcome`、`ConnectionHandle`、`startConnection`、`enqueueSync`；文件顶部注释线索：Connection supervisor: owns the MCP client/transport generations for one plugin instance, keeps the harness tool registry in sync with the live generation, and — when the connection drops — restarts the configured server with bounded exponential backoff. On...。 这些数字和声明用于定位，不替代源码阅读。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：模块入口
- 这个文件有什么用：它把 MCP 连接、浏览器端 相关的公开能力集中导出，并决定调用者可以依赖哪些边界；调用者因此不必记住所有内部文件。
...
- 测试关联依据：固定提交源码中的本地静态 import 直接引用；测试用例直接导入了这个源文件。
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 181 行；扫描到的声明包括 `name`、`inject`、`StdioConfig`、`StreamableHttpConfig`、`Config`、`apply`；文件顶部注释线索：MCP client bridge plugin: connects to an external MCP server and registers its tools on ctx.tools under server-qualified public names (mcp__<serverName>__<rawName>). Each plugin instance connects to one MCP server; load multiple instances in cordis.yml for ...。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：运行时不变量
- 这个文件有什么用：它检查 MCP 连接、浏览器端 必须始终成立的条件，在错误刚出现时报告，而不是等到更深层才出现难以解释的结果。
...
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 30 行；扫描到的声明包括 `name`、`inject`、`apply`；文件顶部注释线索：Package-owned invariant companion for @deepseek-ai/dsh-mcp-client. @module @deepseek-ai/dsh-mcp-client/invariant。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：工具能力
- 这个文件有什么用：它提供 MCP 连接、浏览器端、工具 的一项可调用能力，通常同时处理参数、执行和结果展示；独立工具让权限和测试可以逐项控制。
...
- 测试关联依据：固定提交源码中的本地静态 import 直接引用；测试用例直接导入了这个源文件。
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 317 行；扫描到的声明包括 `ToolBridgeOptions`、`ToolDisposers`、`McpResult`、`publicToolName`、`syncTools`、`listToolsUncached`、`callToolUncached`、`supportedOutputSchema`；文件顶部注释线索：Tool bridge: discovers MCP tools, registers them on the harness ToolRuntime under deterministic server-qualified public names, and handles re-sync when the server's tool list changes. Naming contract (see the mcp-client Agent Note "Naming invariants"): ever...。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：外部能力适配层
- 这个文件有什么用：它把外部协议转换成 MCP 连接、浏览器端 能理解的内部协议。转换集中在边界，核心逻辑就不必到处处理供应商差异。
...
- 测试关联依据：固定提交源码中的本地静态 import 直接引用；测试用例直接导入了这个源文件。
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 50 行；扫描到的声明包括 `createTransport`、`buildChildEnv`；文件顶部注释线索：Transport factory: creates the appropriate MCP transport based on the plugin's resolved config. Stdio spawns a child process (with credential scrubbing); Streamable HTTP connects to a URL. @module。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：测试用例
- 这个文件有什么用：它直接验证 MCP 连接、浏览器端 的成功、失败或边界行为，把“应该发生什么”变成可以重复运行的证据。
...
- 所属层：packages/mcp：可复用的 Harness 功能包
- 文件角色：测试支持
- 这个文件有什么用：它为 MCP 连接、浏览器端 的测试提供共享准备或环境支持；真正的行为断言由导入它的测试用例完成。
...
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 76 行；扫描到的测试主题包括 “First number”、“Second number”、“Name to greet”；文件顶部注释线索：Minimal MCP server over stdio for e2e testing of the dsh-mcp-client plugin. Registers controlled tools with predictable behavior for asserting edge cases. Run: node fixture-server.ts。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
...
- 测试关联依据：本文件本身就是测试用例，不把同目录的其他测试冒充成它的“对应测试”。
- 阅读顺序：先看所在层的说明，再看包 README 和入口，然后读本文件，最后对照测试；本条是自动索引，复杂行为需要回到源码和测试确认。
- 代码证据：固定提交归档实际读取结果：约 29 行；扫描到的测试主题包括 “dsh-mcp-client real-load-path guard”、“has no default export and keeps name/inject/Config through unwrapExports”；文件顶部注释线索：Real-load-path guard for @deepseek-ai/dsh-mcp-client. mcp-client is a NAMESPACE plugin with inject — so a stray export default apply would make the cordis Loader's unwrapExports (exports.default ?? exports) collapse the module to the bare apply function, DR...。 这些数字和声明用于定位，不替代源码阅读。
- 固定版本：源码链接固定到官方提交 `47f943859bef60e4160492346772ded9b24f765a`；如果当前条目与运行版本不同，应先重新生成索引再下结论。
