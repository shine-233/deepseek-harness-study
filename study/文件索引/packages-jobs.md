...
- 文件级设计证据：源码顶部注释把它定位为“Package-owned invariant companion for @deepseek-ai/dsh-jobs-local. @module @deepseek-ai/dsh-jobs-local/invariant”；固定提交中扫描到的声明包括 `name`、`inject`、`apply`；本地静态 import 图显示它直接依赖 2 个源文件，并被 0 个源文件直接引用。
...
- 代码证据：固定提交归档实际读取结果：约 33 行；扫描到的声明包括 `name`、`inject`、`apply`；源码顶部原注释（英文，仅作回查线索）：Package-owned invariant companion for @deepseek-ai/dsh-jobs-local. @module @deepseek-ai/dsh-jobs-local/invariant。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 文件级设计证据：固定提交中扫描到的声明包括 `stubAgent`、`disposeAgentScope`、`producer`、`harness`、`attachControllerIn`；本地静态 import 图显示它直接依赖 6 个源文件，并被 0 个源文件直接引用。
...
- 对应测试：本文件本身就是测试用例。
- 测试关联依据：本文件本身就是测试用例，不把同目录的其他测试冒充成它的“对应测试”。
- 阅读顺序：先看它直接导入的被测实现 `packages/jobs/jobs-local/src/index.ts`、`vendor/cordis/src/index.ts`、`vendor/include/src/index.ts`，再读本文件的测试主题、输入和断言；最后对照测试支持和失败输出。
- 代码证据：固定提交归档实际读取结果：约 66 行；扫描到的测试主题包括 “jobs-local through a real Loader composition”、“applies the provider-owned admission config from a Cordis row”。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 阅读顺序：先读 `packages/jobs/jobs/README.md`、入口和消费者，再读当前契约，沿着 `apps/web/tests/background-job-list.e2e.ts`、`packages/host/apiproxy/src/api-proxy.ts`、`packages/host/apiproxy/tests/api-proxy-jobs.spec.ts` 看它怎样约束运行时，最后对照 `apps/web/tests/background-job-list.e2e.ts`、`packages/host/apiproxy/tests/api-proxy-jobs.spec.ts`、`packages/jobs/jobs-local/tests/jobs.spec.ts`。
- 代码证据：固定提交归档实际读取结果：约 179 行；源码顶部原注释（英文，仅作回查线索）：The background-job Service Definition (ctx.jobs). It owns the contract for job ids, session-scoped access, lifecycle state, completion listeners, and owner cleanup while producers retain their execution resources. The process-local registry lives in @deepse...。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 这个文件有什么用：它检查后台任务必须始终成立的条件，在错误刚出现时报告，而不是等到更深层才出现难以解释的结果。
- 为什么这样设计：把不变量集中在一个位置，调用者和测试就能用同一条规则检查状态；错误在边界处报告，比在后续 UI 或网络请求中才暴露更容易修复。
- 文件级设计证据：源码顶部注释把它定位为“Package-owned background-job snapshot invariants. @module @deepseek-ai/dsh-jobs/invariant”；固定提交中扫描到的声明包括 `name`、`inject`、`apply`、`validateSnapshot`；本地静态 import 图显示它直接依赖 4 个源文件，并被 1 个源文件直接引用。
...
- 测试关联依据：固定提交源码中的本地静态 import 直接引用；测试用例直接导入了这个源文件。
- 阅读顺序：先读相关类型和事件，再读当前状态或存储实现，沿着 `packages/core/agent/src/index.ts`、`packages/jobs/jobs/src/types.ts`、`packages/runtime-diagnostics/invariants/src/index.ts` 和 `packages/jobs/jobs/tests/invariant.spec.ts` 理解状态变化，最后对照 `packages/jobs/jobs/tests/invariant.spec.ts`。
- 代码证据：固定提交归档实际读取结果：约 57 行；扫描到的声明包括 `name`、`inject`、`apply`、`validateSnapshot`；源码顶部原注释（英文，仅作回查线索）：Package-owned background-job snapshot invariants. @module @deepseek-ai/dsh-jobs/invariant。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 文件级设计证据：源码顶部注释把它定位为“Types shared by job producers, the registry, and controllers. The service implementation lives in ./index.ts. @module @deepseek-ai/dsh-jobs/types”；固定提交中扫描到的声明包括 `JobStatus`、`JobKindMap`、`JobKind`、`JobOutcome`、`JobStart`；本地静态 import 图显示它直接依赖 3 个源文件，并被 2 个源文件直接引用。
...
- 测试关联依据：固定提交源码中的本地静态 import 传递引用；这些测试通过包入口或中间模块到达本文件，不等于本文件被直接覆盖。
- 阅读顺序：先读 `packages/jobs/jobs/README.md`、入口和消费者，再读当前契约，沿着 `packages/jobs/jobs/src/index.ts`、`packages/jobs/jobs/src/invariant.ts` 看它怎样约束运行时，最后对照 `apps/cli/tests/web-agent-presets.e2e.ts`、`apps/web/tests/agent-preset-selection.e2e.ts`、`apps/web/tests/background-job-list.e2e.ts`。
- 代码证据：固定提交归档实际读取结果：约 160 行；扫描到的声明包括 `JobStatus`、`JobKindMap`、`JobKind`、`JobOutcome`、`JobStart`、`JobHooks`、`JobSnapshot`、`JobRead`；源码顶部原注释（英文，仅作回查线索）：Types shared by job producers, the registry, and controllers. The service implementation lives in ./index.ts. @module @deepseek-ai/dsh-jobs/types。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 代码证据：固定提交归档实际读取结果：约 87 行；扫描到的声明包括 `setup`；扫描到的测试主题包括 “job-registry invariants”、“accepts coherent current and terminal snapshots”、“rejects an incoherent record already present at installation”。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 文件级设计证据：固定提交中扫描到的声明包括 `StubJobRegistry`、`SecondJobRegistry`；本地静态 import 图显示它直接依赖 3 个源文件，并被 0 个源文件直接引用。
...
- 对应测试：本文件本身就是测试用例。
- 测试关联依据：本文件本身就是测试用例，不把同目录的其他测试冒充成它的“对应测试”。
- 阅读顺序：先看它直接导入的被测实现 `packages/core/agent/src/index.ts`、`packages/jobs/jobs/src/index.ts`、`vendor/cordis/src/index.ts`，再读本文件的测试主题、输入和断言；最后对照测试支持和失败输出。
- 代码证据：固定提交归档实际读取结果：约 96 行；扫描到的声明包括 `StubJobRegistry`、`SecondJobRegistry`；扫描到的测试主题包括 “JobRegistry seam”、“a concrete subclass registers as ctx.jobs and serves the abstract API”、“loading a second implementation throws (one jobs service per context — cordis standard)”、“mounting the abstract seam directly fails loudly at load (stale-composition fence)”。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 代码证据：固定提交归档实际读取结果：约 402 行；扫描到的声明包括 `name`、`inject`、`CompletionDelivery`、`Config`、`PublicJobSnapshot`、`statusLine`、`apply`、`publicJob`；源码顶部原注释（英文，仅作回查线索）：Model-facing job_output, job_list, and job_kill tools over ctx.jobs. Loading the plugin attaches the controller required by producers. It also delivers unreported completions to the owning agent: injected into a busy owner's next step, or opening a turn on ...。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
...
- 为什么这样设计：把不变量集中在一个位置，调用者和测试就能用同一条规则检查状态；错误在边界处报告，比在后续 UI 或网络请求中才暴露更容易修复。
- 文件级设计证据：源码顶部注释把它定位为“Package-owned invariant companion for @deepseek-ai/dsh-tool-jobs. @module @deepseek-ai/dsh-tool-jobs/invariant”；固定提交中扫描到的声明包括 `name`、`inject`、`apply`；本地静态 import 图显示它直接依赖 2 个源文件，并被 0 个源文件直接引用。
- 直接协作者：[packages/jobs/tool-jobs/README.md](https://github.com/deepseek-ai/deepseek-harness/blob/aa6c361a972c8369148dea7380bb5c21c24e07ec/packages/jobs/tool-jobs/README.md)、[packages/runtime-diagnostics/invariants/src/index.ts](https://github.com/deepseek-ai/deepseek-harness/blob/aa6c361a972c8369148dea7380bb5c21c24e07ec/packages/runtime-diagnostics/invariants/src/index.ts)、[vendor/cordis/src/index.ts](https://github.com/deepseek-ai/deepseek-harness/blob/aa6c361a972c8369148dea7380bb5c21c24e07ec/vendor/cordis/src/index.ts)
- 对应测试：没有确认到直接测试；公共入口可能仍有间接覆盖。
- 测试关联依据：固定提交中没有找到直接或传递的本地静态 import，也没有找到明显的同包同名测试；公共入口可能仍有间接覆盖。
- 阅读顺序：先读相关类型和事件，再读当前状态或存储实现，沿着 `packages/runtime-diagnostics/invariants/src/index.ts`、`vendor/cordis/src/index.ts` 和所在包的入口或服务理解状态变化，最后对照同包中与它同名或覆盖相近场景的测试。
- 代码证据：固定提交归档实际读取结果：约 30 行；扫描到的声明包括 `name`、`inject`、`apply`；源码顶部原注释（英文，仅作回查线索）：Package-owned invariant companion for @deepseek-ai/dsh-tool-jobs. @module @deepseek-ai/dsh-tool-jobs/invariant。
- 固定版本：源码链接固定到官方提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`。
