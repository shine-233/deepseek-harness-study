...
- 阅读顺序：先读 `packages/jobs/jobs` 的入口和消费者，再读当前契约，沿着 `apps/web/tests/background-job-list.e2e.ts`、`packages/host/apiproxy/src/api-proxy.ts`、`packages/host/apiproxy/tests/api-proxy-jobs.spec.ts` 看它怎样约束运行时，最后对照 `apps/web/tests/background-job-list.e2e.ts`、`packages/host/apiproxy/tests/api-proxy-jobs.spec.ts`、`packages/jobs/jobs-local/tests/jobs.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 测试关联依据：固定提交源码中的本地静态 import 传递引用；这些测试通过包入口或中间模块到达本文件，不等于本文件被直接覆盖。
- 阅读顺序：先读 `packages/jobs/jobs` 的入口和消费者，再读当前契约，沿着 `packages/jobs/jobs/src/index.ts`、`packages/jobs/jobs/src/invariant.ts` 看它怎样约束运行时，最后对照 `apps/cli/tests/web-agent-presets.e2e.ts`、`apps/web/tests/agent-preset-selection.e2e.ts`、`apps/web/tests/background-job-list.e2e.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
