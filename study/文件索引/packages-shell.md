...
- 文件级设计证据：源码顶部注释把它定位为“PowerShell executable resolution, dependency-free so non-package consumers (the repository's coverage-gate probe in vitest.config.ts) can share the ONE resolution definition with the executor and its suites — a probe that resolved differently from the code ...”；固定提交中扫描到的声明包括 `candidatePwshPaths`、`resolvePwshPath`、`candidateExists`；本地静态 import 图显示它直接依赖 0 个源文件，并被 2 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 阅读顺序：先读 `packages/shell/pwsh-local/README.md` 和入口，再读当前实现，沿着相关类型、协议或实现和 `packages/shell/pwsh-local/src/index.ts`、`vitest.config.ts` 确认输入输出，最后对照 `apps/web/tests/pwsh-terminal.e2e.ts`、`examples/acp-agent/tests/acp.snapshot.ts`、`packages/core/tools/tests/gen-tool-catalog.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
