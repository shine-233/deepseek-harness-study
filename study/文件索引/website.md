...
- 阅读顺序：先读 `website` 的 README 或发布说明，再读本配置/脚本，沿着所在包的入口或服务确认它如何影响入口和产物，最后对照对应 gate 或快照测试。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 阅读顺序：先读 `website` 的 README 或发布说明，再读本配置/脚本，沿着 `scripts/project-doc-site.spec.ts`、`scripts/project-doc-site.ts`、`website/.vitepress/config.ts` 确认它如何影响入口和产物，最后对照对应 gate 或快照测试。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
