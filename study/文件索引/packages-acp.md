...
- 阅读顺序：先读 `packages/acp/acp` 的 README 和入口，再读当前实现，沿着 `packages/core/session/src/index.ts` 和 `packages/acp/acp/src/index.ts`、`packages/acp/acp/tests/codec.spec.ts` 确认输入输出，最后对照 `packages/acp/acp/tests/codec.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 阅读顺序：先读 `packages/acp/acp` 的入口和消费者，再读当前契约，沿着 `packages/acp/acp/tests/harness.ts`、`packages/examples/acp-demo/src/index.ts` 看它怎样约束运行时，最后对照 `packages/acp/acp/tests/approval.spec.ts`、`packages/acp/acp/tests/bridge.spec.ts`、`packages/acp/acp/tests/dispose.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
