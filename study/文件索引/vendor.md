...
- 为什么这样设计：Vendored 源码单独放置，便于记录来源、版本、许可证和 DSH 的本地修改；它不是未经维护的黑盒上游，学习时要同时区分上游概念与 DSH 固定的 fork 行为。
- 文件级设计证据：源码顶部注释把它定位为“Core context type and root context implementation.”；本地静态 import 图显示它直接依赖 7 个源文件，并被 1046 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 为什么这样设计：Vendored 源码单独放置，便于记录来源、版本、许可证和 DSH 的本地修改；它不是未经维护的黑盒上游，学习时要同时区分上游概念与 DSH 固定的 fork 行为。
- 文件级设计证据：固定提交中扫描到的声明包括 `entryListSchema`、`applyEntryPatches`、`PatchOptions`、`Config`、`Include`；本地静态 import 图显示它直接依赖 2 个源文件，并被 42 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 为什么这样设计：Vendored 源码单独放置，便于记录来源、版本、许可证和 DSH 的本地修改；它不是未经维护的黑盒上游，学习时要同时区分上游概念与 DSH 固定的 fork 行为。
- 文件级设计证据：固定提交中扫描到的声明包括 `Config`、`Intercept`、`Loader`；本地静态 import 图显示它直接依赖 8 个源文件，并被 88 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 为什么这样设计：Vendored 源码单独放置，便于记录来源、版本、许可证和 DSH 的本地修改；它不是未经维护的黑盒上游，学习时要同时区分上游概念与 DSH 固定的 fork 行为。
- 文件级设计证据：固定提交中扫描到的声明包括 `From`、`TypeS`、`TypeT`、`Resolve`、`IntersectS`；本地静态 import 图显示它直接依赖 1 个源文件，并被 127 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
