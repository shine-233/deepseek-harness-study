...
- 阅读顺序：先读 `packages/credentials/credentials-local/README.md`、入口和消费者，再读当前契约，沿着 `packages/credentials/credentials-local/tests/drain.spec.ts`、`packages/credentials/credentials-local/tests/local.spec.ts`、`packages/credentials/credentials-local/tests/review-fixes.spec.ts` 看它怎样约束运行时，最后对照 `packages/credentials/credentials-local/tests/drain.spec.ts`、`packages/credentials/credentials-local/tests/local.spec.ts`、`packages/credentials/credentials-local/tests/review-fixes.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 文件级设计证据：源码顶部注释把它定位为“Third-review behaviors: read-modify-write under the writer lock (external edits survive an API write), the contained credentials/updated fan-out (a broken observer never fails a committed write), and the YAML document editor's isolation between entries.”；固定提交中扫描到的声明包括 `writeCredentials`、`tempDir`、`boot`；本地静态 import 图显示它直接依赖 3 个源文件，并被 0 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 阅读顺序：先读 `packages/credentials/credentials/README.md`、入口和消费者，再读当前契约，沿着 `apps/web/tests/web-search-round.e2e.ts`、`packages/credentials/credentials-local/src/index.ts`、`packages/credentials/credentials-local/tests/drain.spec.ts` 看它怎样约束运行时，最后对照 `apps/web/tests/web-search-round.e2e.ts`、`packages/credentials/credentials-local/tests/drain.spec.ts`、`packages/credentials/credentials-local/tests/local.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 文件级设计证据：源码顶部注释把它定位为“Client-safe type surface of the credential-reference seam: the reference brand and the seam's Cordis event declaration. Types only — no runtime code, and nothing here reaches a Host-only symbol, so a Client compilation face reads exactly the signature the H...”；固定提交中扫描到的声明包括 `CredentialRef`；本地静态 import 图显示它直接依赖 1 个源文件，并被 3 个源文件直接引用。这些是文件级定位证据，用来约束“为什么这样设计”的解释范围；它们仍不替代人工源码阅读。
...
- 阅读顺序：先读 `packages/credentials/credentials/README.md`、入口和消费者，再读当前契约，沿着 `packages/api/remotes/src/client/index.ts`、`packages/api/remotes/src/index.ts`、`packages/credentials/credentials/src/index.ts` 看它怎样约束运行时，最后对照 `apps/web/tests/web-search-round.e2e.ts`、`examples/headless-agent/tests/code-mode.e2e.ts`、`examples/headless-agent/tests/coding-task.e2e.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
