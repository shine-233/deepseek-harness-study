...
- 所属层：packages/feedback：可复用的 Harness 功能包
- 文件角色：功能实现
- 这个文件有什么用：它位于 `packages/feedback/message-feedback`，围绕数据规格（`spec`）组织实现；固定提交中可见的公开或顶层声明包括 `messageFeedbackRatingSchema`、`messageFeedbackVersionSchema`、`messageFeedbackItemSchema`、`messageFeedbackSessionIdentitySchema`、`MessageFeedbackSessionIdentity`，这些声明构成它对外提供的主要入口。阅读时应沿直接协作者和测试继续确认具体输入、输出与失败边界。
- 为什么这样设计：把数据规格（`spec`）单独放在 `packages/feedback/message-feedback`，可以让这一段实现拥有清楚的输入、输出和替换边界；固定版本中它连接 4 个本地依赖和 1 个直接使用者，读者可以沿这些连接验证设计是否成立。
...
- 阅读顺序：先读 `packages/feedback/message-feedback/README.md` 和入口，再读当前实现，沿着 `packages/core/session/src/types.ts`、`packages/feedback/message-feedback/src/types.ts`、`packages/llm/llm/src/brand.ts` 和 `packages/feedback/message-feedback/src/index.ts` 确认输入输出，最后对照 `packages/feedback/message-feedback/tests/invariant.spec.ts`、`packages/feedback/message-feedback/tests/loader-composition.spec.ts`、`packages/feedback/message-feedback/tests/message-feedback.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
