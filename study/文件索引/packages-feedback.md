...
- 阅读顺序：先读 `packages/feedback/command-feedback` 的入口和消费者，再读当前契约，沿着 `examples/headless-agent/tests/fixtures/session-telemetry-otel-driver.ts`、`packages/feedback/command-feedback/tests/command-feedback.spec.ts`、`packages/feedback/command-feedback/tests/loader-composition.spec.ts` 看它怎样约束运行时，最后对照 `packages/feedback/command-feedback/tests/command-feedback.spec.ts`、`packages/feedback/command-feedback/tests/loader-composition.spec.ts`、`packages/session/session-telemetry-otel/tests/otel.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 阅读顺序：先读 `packages/feedback/message-feedback` 的 README 和入口，再读当前实现，沿着 `packages/core/session/src/types.ts`、`packages/feedback/message-feedback/src/types.ts`、`packages/llm/llm/src/brand.ts` 和 `packages/feedback/message-feedback/src/index.ts` 确认输入输出，最后对照 `packages/feedback/message-feedback/tests/invariant.spec.ts`、`packages/feedback/message-feedback/tests/loader-composition.spec.ts`、`packages/feedback/message-feedback/tests/message-feedback.spec.ts`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
...
- 测试关联依据：固定提交源码中的本地静态 import 直接引用；测试用例直接导入了这个源文件。
- 阅读顺序：先读 `packages/feedback/message-feedback` 的入口和消费者，再读当前契约，沿着 `packages/client/ui-message-feedback/src/client/MessageFeedbackActions.tsx`、`packages/client/ui-message-feedback/src/client/controller.ts`、`packages/client/ui-message-feedback/src/client/slots.ts` 看它怎样约束运行时，最后对照 `packages/client/ui-message-feedback/tests/browser-plugin.client.spec.tsx`、`packages/client/ui-message-feedback/tests/controller.client.spec.ts`、`packages/client/ui-message-feedback/tests/message-feedback-actions.client.spec.tsx`。自动索引只提供定位线索，复杂行为需要回到源码和测试确认。
