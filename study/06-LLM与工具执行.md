...
这里要多记一层：工具注册表里的全集，不等于当前 agent 发给模型的集合。官方系统提示词组装会收集每个 agent 可见的 schema；限制工具会移除该 agent 的整段 schema 成本，但不等于操作系统权限被隔离。工具数量、description、参数和枚举可能增加输入成本，真实延迟仍需按模型和 provider 实测。
