...
第一次运行时，你可以把输出和[工具可见集合观测与性能实验](23-工具可见集合观测与性能实验.md)里的实测记录对照。当前最新记录使用 Node `v24.15.0`、Windows `win32`、`x64`，两组都注册 24 个工具；A 组可见 24 个，B 组通过同一个 agent 作用域的 `restrict({ allow })` 可见 3 个。
...
绝对时间会随机器、JIT 和负载变化，读者应把它当作本机相对记录，不是性能承诺。
