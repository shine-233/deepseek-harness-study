...
一个插件函数被调用，不代表它只做了一次计算。它可能注册事件监听器、创建 Service、启动定时器、加载子插件、等待依赖或打开文件。Fiber 会记录这些动作产生的可撤销效果，并在插件离开树时按正确顺序清理。
...
- 卸载时要清理事件、服务、子插件和其他 Effect。
- 重复 dispose 必须保持幂等，清理过程中不能偷偷注册逃出生命周期的新 Effect。
- 配置更新需要先处理候选状态，失败时尽量恢复之前仍然有效的插件树。

所以 Fiber 不是普通的“插件状态变量”，它是所有权边界。谁创建了一个资源，谁的 Fiber 就应该能负责它的清理。
