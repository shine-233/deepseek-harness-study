...
“万物皆插件”是可以数出来的：下面这个组件读取固定提交 `aa6c361a97` 的清单和源码行数。结果是 227 个包分在 50 个组里，包与包之间有 1124 条 `peerDependencies` 依赖。
...
不打开组件也能知道答案：在固定提交里，`runtime-diagnostics/invariants` 只有 230 行，却被 226 个包依赖；`client/runtime` 有 9032 行，只被 37 个包依赖。体量和被依赖程度是两件事，所以“哪个包重要”不能按行数排。组件里的每个数字都在它自己的表格里逐行给出。
