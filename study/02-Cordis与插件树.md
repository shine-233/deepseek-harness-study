...
不打开组件也能知道答案：在固定提交里，`runtime-diagnostics/invariants` 只有 230 行，却被 218 个包依赖；`client/runtime` 有 8989 行，只被 32 个包依赖。体量和被依赖程度是两件事，所以“哪个包重要”不能按行数排。组件里的每个数字都在它自己的表格里逐行给出。
