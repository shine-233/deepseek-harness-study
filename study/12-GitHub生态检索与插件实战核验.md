...
固定 DSH 提交中被本仓库纳入的 2,973 个代码或界面源文件，是否真的每一条都有中文索引条目？GitHub 上看起来像 DSH 插件的仓库，又各自属于哪一类？这一章把这两个问题分开回答。

先给结论：固定 DSH 提交中被本仓库纳入的 2,973 个代码或界面源文件，确实都有一条对应的中文索引条目；但这不等于 2,973 个文件都被人工逐行精读，也不等于所有 JSON、YAML、Markdown、快照和资源文件都被遗漏。它们是另一类重要材料，当前不计入“逐源码文件”数字。

本章所有数字和样本共用同一条边界：2,973 是静态证据驱动的索引条目数，不是人工逐行审查数；GitHub `topic:dsh-plugin` 的计数是一次公开检索快照，只覆盖公开 repository 元数据中被 topic 标记的候选集合，不包括私有仓库、已删除仓库、没有 topic 的项目和只存在于代码或 issue 中的项目，也不能证明候选能被 DSH 安装，所以不能写成“GitHub 上有 N 个插件”；registry 的 `verifiedAgainst` 是维护者自记字段，不等于官方认证；`dsh-super-injector` 的低层注入能力是边界案例，不等于官方公开热插拔 API；普通插件、Bundle、Skill、MCP、preset、市场、管理器、桌面 wrapper 和运行时注入器解决不同问题，必须分开描述；本仓库没有对所有第三方项目做真实安装、真实模型请求、三平台兼容性和卸载验证，搜索结果只当候选发现数据，不当官方注册表。

从候选仓库判断它到底是什么的四步判读在第四节只是执行摘要；打开仓库后的完整六步核验流程见[社区生态与扩展边界](10-社区生态与扩展边界.md)。

## 一、怎样证明“每个源文件都有对应材料”

本仓库固定的上游提交是 [`aa6c361a972c8369148dea7380bb5c21c24e07ec`](https://github.com/deepseek-ai/deepseek-harness/tree/aa6c361a972c8369148dea7380bb5c21c24e07ec)。这个提交的 Git tree 有 7,903 个文件，生成器按照源码扩展名白名单识别出 2,973 个代码或界面源文件，分到 `study/文件索引/` 的 78 个索引页中。

每条索引有十一条必填字段和可选的「测试支持」字段，完整定义见[逐文件索引怎么读](08-逐文件索引怎么读.md)，此处不再复制。
...
下面的项目是本次只读核验的代表性样本；链接固定到本次读取的分支 HEAD，方便以后比较——“固定 commit”只固定当时看的内容，不代表项目未来仍然兼容。
...
这个顺序就是设计边界：越靠前越可能作出决策，越靠后越接近不可变的观察结果。选择扩展点时，先问“我想观察、决策、包裹执行，还是提供能力”，再看对应 API。「正常 agent 只保留最小可见集合」的收窄过程，可以在[工具可见性实验](/tool-visibility-lab.html)里用 12 个教学工具亲手勾一遍，看每个工具被 Bundle、作用域还是策略挡住。
...
先看[逐文件索引的读法](08-逐文件索引怎么读.md)确认静态证据，再看[社区生态与扩展边界](10-社区生态与扩展边界.md)理解分类，最后看[如何写一个合规插件](11-如何写一个合规插件.md)写最小包；本章适合在准备安装社区项目或建立自己的 registry 前回看。
