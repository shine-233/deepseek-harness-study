...
各类组件分别打到循环的哪一层——Skill 渐进加载改上下文与观测、Browser use 扩大动作与观测、记忆改长程状态、RAG 改证据输入、工具延迟加载改候选集合、自动修改改控制策略——逐项对照表和「第一批该观察什么」在[渐进式设计](32-源码学习项目的渐进式设计.md)。这里只立一条规矩：组件让循环变复杂，不等于效果已经变好。

因此，“纯软 Agent 基本到头”不适合作为教材中的绝对结论：朴素的单循环方案遇到的是边际收益和验证瓶颈，分层 Agent、Planner／Executor、事件驱动流程、状态机、多 Agent 和外部执行环境仍然有研究价值。“没有大量数据就几乎不能验证”也要改准：没有大规模用户数据时，仍然可以用固定任务集、离线 replay、确定性夹具和小型配对 A/B 验证机制是否工作、是否回归；缺的是把结果推广到异质真实用户群体的统计把握，不是完全不能验证。

用户数据不能直接当成无偏实验：任务难度、用户熟练度、模型版本、Provider、工具集合、延迟、配置和成功样本选择都会混在一起，只看“某次改完成功了”无法知道收益来自哪一项变化。要回答“某次改动造成了多少提升”，已经是因果比较而不是普通日志统计——配对或交错 A/B 是最低起点，还要保存任务分层、版本、配置和失败样本；只有相关日志时，写“观察到关联”，不要写“改动带来了提升”。
...
<div class="dsh-research" data-dsh-research aria-label="研究优先级看板：按 P0/P1/P2 筛选，点选研究项看理由、所需资源和验收证据"></div>
...
宿主要明确观测接口是调试能力，不是权限授予；它也不改变[第 22 课](22-工具可见性与非侵入扩展.md)划出的隔离边界。需要访问私有内部状态时，维护者要负责版本锁定、权限说明、回滚步骤和与普通插件 API 的区分。

## P1：社区项目审核顺序

先看项目身份和固定 commit，再看 `package.json`、Bundle manifest、安装脚本和依赖；随后搜索模块缓存、Loader、registry、进程注入、Windows junction/注册表、网络和子进程行为。

再把“项目自称支持”按[决策卡的证据分级](27-工具预算与插件责任决策卡.md)拆层核对：从 README 声明到卸载回滚，每一层能证明的东西不同；审核记录要写清这次拿到了哪一层、还缺哪一层。
