...
| 评估闭环 | 用固定任务和指标判断改动是否有收益 | 对照组、实验组、保留集和护栏指标 |
...
最小的可信门槛是：固定 Harness 版本、固定模型和 Provider、固定分层任务、预先定义主指标和护栏指标、保留不可见的 holdout 集、交错或配对 A/B、记录环境、人工批准推广，并能自动回滚。
...
| 观察对象 | 已有证据 | 证据等级 | 仍未验证 |
| --- | --- | --- | --- |
| 上游 Hook bridge | 固定提交 `47f943859bef60e4160492346772ded9b24f765a` 的 `hook-protocol` README 写明 library 不注册、不注入；Claude/Codex bridge README、源码和测试分别列出 7/5 个事件子集。见[协议 README][hook-protocol-readme]、[Claude bridge README][claude-bridge-readme]、[Codex bridge README][codex-bridge-readme]。 | 固定上游源码/测试 | 未运行本轮 DSH、外部 shell hook 或模型验证；未来协议字段和支持子集仍需重查。 |
...
- [给官方 DSH 的下一步更新建议](./33-给官方DSH的更新建议.md)
...
[claude-bridge-readme]: https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/hooks/hooks-claude-code/README.md
[codex-bridge-readme]: https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/hooks/hooks-codex/README.md
