...
不打开组件也能带走三条结论。第一，订阅决定收不收得到：未订阅的插件照样被挂载，但广播不会送到它面前，预览数为零；「中途订阅」场景还能看到登记之前的广播同样收不到——订阅时机决定错过什么。第二，日志跟订阅无关：`tool/call` 和 `tool/result` 由宿主写入，策略拒绝甚至卸载插件都不影响日志完整性。第三，注册与注销必须成对：结束时活跃效果数不为零，就说明有监听器还挂着。时间线支持逐步推进：滑杆停在任意一步，看这一步谁在做动作、日志里有没有多一行。
