...
<LessonWidget
  id="plugin-flow-lab"
  url="/plugin-flow-lab.html"
  title="插件事件流实验室"
  :height="1000"
  fallback-href="#第一个例子只观察工具结果"
>

不打开组件也能带走三条结论。第一，订阅决定收不收得到：未订阅的插件照样被挂载，但广播不会送到它面前，预览数为零。第二，日志跟订阅无关：`tool/call` 和 `tool/result` 由宿主写入，策略拒绝甚至卸载插件都不影响日志完整性。第三，注册与注销必须成对：结束时活跃效果数不为零，就说明有监听器还挂着。
...
把 `maxLength` 改成 `6` 再运行一次；把 `observer.setup()` 注释掉再运行——挂载了但没订阅，广播从它面前经过。
