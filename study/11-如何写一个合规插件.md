...
这个插件不改变工具行为，只在工具结果完成后打印一行日志。它适合作为第一步，因为它没有权限决策，也不需要修改 agent loop。
...
// `tools/result` 由 dsh-tools 声明和发出；注入可以让插件只在工具服务就绪后运行。
...
export function apply(ctx) {
  ctx.on('tools/result', (exec, result) => {
    const text = result.content
      ...
      .join('')
    console.log(`[study-plugin] ${exec.name} -> ${text}`)
  })
}
```

这里的 `ctx.on()` 是 Cordis 的公开注册方法。监听器属于当前插件 Fiber，卸载 Fiber 时会自动移除；插件不需要访问 `ctx.events._hooks`，也不需要自己从数组中删除监听器。

`inject` 表示硬依赖，而不是加载顺序。Loader 可以并发声明多个插件，Cordis 会等 `tools` provider 就绪后再运行本插件；如果 provider 消失，消费方也会按生命周期撤销并在依赖恢复后重新加载。
...
Cordis 已经替你管理的注册，不要重复写清理逻辑。`ctx.on()`、`ctx.tools.register()`、`ctx.provide()` 和 `ctx.plugin()` 都会把所有权挂到当前 Fiber。
...
DSH 官方测试规范还要求产品可见插件使用真实组合测试，而不是只手工创建一个 Context 并断言某个函数被调用。最小 Context 测试适合快速定位；Loader 启动的组合测试才验证了 package manifest、配置层和实际依赖关系。
...
- [Cordis 第一个插件教程](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cordis-tutorial/01-first-plugin.zh.md)
- [Cordis 生命周期与 effect](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cordis-tutorial/02-lifecycle-and-effects.zh.md)
- [Cordis 服务教程](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cordis-tutorial/03-services.zh.md)
- [Cordis 事件教程](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cordis-tutorial/04-events.zh.md)
- [进入 harness 的工具教程](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cordis-tutorial/07-into-the-harness.zh.md)
- [扩展插件手册](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/cookbook/extension-cookbook.zh.md)
- [插件打包与安装](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/docs/user/develop/basic/publish.zh.md)
- [`dsh-tools` 的扩展点说明](https://github.com/deepseek-ai/deepseek-harness/blob/47f943859bef60e4160492346772ded9b24f765a/packages/core/tools/README.zh.md)
