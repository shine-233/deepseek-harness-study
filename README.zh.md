...
>
> 只看 GitHub 网页就能开始：不需要下载源码、不需要 API key、不需要安装 pnpm，也不需要先会 TypeScript。进入后按“第一课 → 仓库地图 → 选择一条路线”走；暂时不要点 `Code`，也不要从 2,973 个逐文件索引条目开始。
>
...
> **已经在 Codespaces：** 在仓库根目录运行 `pnpm run study:quick-check`，可以一次检查学习入口、离线快照和固定源码链接；加 `-- --example` 会再检查最小示例的 test/lint；Pages 构建后还可以加 `-- --site` 检查每个学习页的 HTML 外壳和站内链接。它不启动 DSH、provider 或模型。这条入口还会检查关键页面的学习体验契约：页面必须保留“现在做什么、应该看到什么或没有证明什么”的提示。
...
本仓库把官方 DSH（DeepSeek Harness）固定在提交 `aa6c361a972c8369148dea7380bb5c21c24e07ec`，并增加中文源码导读。目标是让第一次接触大型 TypeScript 项目的人，也能知道每一层、每个关键文件和每个源文件索引条目分别解决什么问题。

## 起步三步

1. 打开[开始学习入口：START-HERE.md](START-HERE.md)，它会根据你的目标告诉你下一步点哪一篇。
2. 第一次只读[00-开始这里](study/00-开始这里.md)、[01-仓库地图](study/01-仓库地图.md)和[02-Cordis与插件树](study/02-Cordis与插件树.md)。
...
不要从 2,973 个索引条目第一页开始顺序阅读。先用 `START-HERE.md` 选目标，再用索引把概念落回固定版本源码。

## 先从这里开始

- [开始学习入口](START-HERE.md)：只回答“我现在点哪里、第一轮读什么、什么时候需要下载或开云端”。
- [从零开始的学习路线](study/00-开始这里.md)：先读本教材对 Agent loop、Harness、工具和评估边界的总论，再建立“插件、服务、事件、轮次、会话”的整体认识。
...
当前固定提交中按生成器的源码扩展名白名单识别出 2,973 个代码或界面源文件，包括 TypeScript、TSX、CSS、SQL、JavaScript、Python、C/C++、Shell 和 HTML；测试文件也包括在内。它们被分到 `study/文件索引/` 的 67 个索引页中，每个文件都有单独的中文条目，固定包含十一个字段：所属层、文件角色、用途、设计原因、文件级设计证据、直接协作者、对应测试、测试关联依据、阅读顺序、代码证据和固定版本。JSON、YAML、Markdown、JSONL、锁文件和资源仍然是官方仓库的重要文件，只是当前没有被计入这组“源码文件”索引。
...
- `vendor/` 是 Cordis、Schemastery 等第三方项目的固定副本，但官方 DSH 对它们做了重命名、构建配置和部分行为修改。索引会列出它们；精读时先看固定版本的 [`vendor/README.md`](https://github.com/deepseek-ai/deepseek-harness/blob/aa6c361a972c8369148dea7380bb5c21c24e07ec/vendor/README.md) 的 Manifest 和 Local modifications，不能把每项实现都直接归因于上游，也不能把它们全部当成 DSH 原创代码。
...
发布门禁会进一步检查首页三条按钮、第一轮目标卡、START-HERE 第一课、最小示例页、学习工具箱和 `reading.css` 是否真的出现在 `website/.dist`，再枚举整套 105 个中文学习页面，并核对构建后的站内学习链接。当前这组构建证据检查了 12,795 条学习路线链接；它只证明“教材被完整投影并且链接能解析”，不冒充浏览器点击、真实 Loader、模型调用或安全审计。首页状态条会把这组静态证据压缩成新手能读懂的四个数字，并由首页状态数字门禁和入口门禁共同核对它是否仍然与当前清单一致。

如果需要终端，打开 Codespaces 后会自动读取仓库的 `.devcontainer/devcontainer.json`：它固定 Node 24、启用 pnpm 11、安装锁定依赖，并转发开发预览 `5173` 和发布预览 `4173`。第一次仍建议先运行 `pnpm run study:quick-check`；这只是学习仓库和文档门禁，不会替你启动 DSH、provider 或模型。
...
```sh
node study-tools/generate-source-index.mjs --commit aa6c361a972c8369148dea7380bb5c21c24e07ec --source-root <固定提交的完整源码目录>
```
