...
- `ctx.tools.debugSnapshot(scope?: ScopeKey): ToolRuntimeDebugSnapshot`：返回深度冻结、可 JSON 序列化的观测结果：精确层的 `registered` 名称、限制前的 `known` 名称、限制与呈现 transport 加入后的运行时 `visible` 名称、`hiddenByRestriction`、有效 `presentationMode`，以及实际面向模型的 wire projection（线投影）的逐 schema／总 UTF-8 字节数。Code Mode 下，`visible` 可能包含 SDK 可到达的工具，而 `visibleSchemas` 可能只包含 `run_code`；快照只返回名称和成本，不暴露 `execute`、呈现回调、参数、凭据或用户内容。它不会修改注册表、执行工具或替换提示词组装；字节数是紧凑 `JSON.stringify` 的大小，不是 provider tokenizer（服务商分词器）的 token 数。
