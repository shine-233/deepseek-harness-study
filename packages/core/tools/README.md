...
- `ctx.tools.debugSnapshot(scope?: ScopeKey): ToolRuntimeDebugSnapshot` Returns a deeply frozen, JSON-serializable observation: exact-layer `registered` names, pre-restriction `known` names, effective runtime `visible` names, `hiddenByRestriction`, the effective `presentationMode`, and per-schema/total UTF-8 byte counts for the actual model-facing wire projection. In Code Mode, `visible` includes SDK-reachable tools while `visibleSchemas` may contain only `run_code`; the snapshot reports names and costs only and never exposes `execute`, presenter callbacks, arguments, credentials, or user content. It does not change the registry, execute a tool, or replace prompt assembly, and the byte count is compact `JSON.stringify` size rather than provider tokenization.
